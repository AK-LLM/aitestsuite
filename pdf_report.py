...
class PromptReportPDF(FPDF):
    def __init__(self, *args, **kwargs):
        ...
        # Use Arial/Helvetica as default font, no need for external TTF
    ...
    def add_prompt_result(self, idx, prompt, result):
        ...
        if result:
            ...
            self.multi_cell(0, 6, f"AI Response: {answer}")
            self.multi_cell(0, 6, f"Risk: {risk}")
            self.multi_cell(0, 6, f"Evidence: {evidence}")
            self.multi_cell(0, 6, f"Recommendations: {recommendation}")
        else:
            self.set_font("Helvetica", "I", 10)
            self.multi_cell(0, 6, "No result available.")
        self.ln(2)
...
def generate_report(results):
    ...
    results = [
        {
            ...
            "recommendations": "Fact-check, improve prompt"
        },
        ...
    ]
    """
    ...
    # Return PDF as bytes for Streamlit's download_button
